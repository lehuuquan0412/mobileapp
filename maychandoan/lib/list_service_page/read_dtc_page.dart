...
class ReadDTCPage extends StatefulWidget {
  @override
  _ReadDTCPageState createState() => _ReadDTCPageState();
}

class _ReadDTCPageState extends State<ReadDTCPage> {
  final databaseRef = FirebaseDatabase.instance.ref('errorCodes');
            ...
            content: SingleChildScrollView(
              child: ListBody(
                children: <Widget>[
                  ...
                  SizedBox(height: 20),
                  Text(
                      'Repair Tips: ${data['repair tips'] ?? 'No Repair Tips'}'),
                ],
              ),
            ),
            actions: <Widget>[
  ...
  void readData() {
    ...
    databaseRef.once().then((DatabaseEvent event) {
      if (event.snapshot.exists) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('No Stored DTC Found',
              style: TextStyle(
                  fontFamily: 'Times', // Specify the font family
                  fontSize: 18, // Specify the font size
                  color: Colors.white // Optional: Change text color
  ...
  void clearData() {
    databaseRef.remove().then((_) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('All Stored DTC have been cleared from Firebase.',
            style: TextStyle(
                fontFamily: 'Times', // Specify the font family
                fontSize: 18, // Specify the font size
                color: Colors.white // Optional: Change text color
  ...
  String extractDTCCode(String fullCode) {
    if (fullCode.startsWith('{DTC: ') && fullCode.endsWith('}')) {
      return fullCode.substring(6, fullCode.length - 1).trim();
    }
    return fullCode;
  }
}
